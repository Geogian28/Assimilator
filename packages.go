...
func (p *packageInfo) ProcessPackage(a *AgentData) error {
	...
	Trace(5, "Successfully ensured ", p.name)

	if !appConfig.RunOnce {
		p.checkLastRunTime()
		Info(2, p.printTimeSinceLastRun())
		// Check if no updates exist AND we are still within the cooldown window
		Trace(5, "p.lastRunTime: ", p.lastRunTime)
		switch {
		case p.updated:
			Info(2, "Updates exist for ", p.name, ". Running...")
			// case p.lastRunTime.IsZero():
			// Info(2, "No last run time for ", p.name, ". Running...")
		case time.Since(p.lastRunTime) < time.Duration(p.updateInterval)*time.Second:
			Info(2, "No updates for ", p.name, " and not enough time has passed since the last run. Skipping.")
			return nil
		}
	}
	...
	Trace(5, "Successfully extracted ", p.name)
...
func (p *packageInfo) ensurePackage(a *AgentData) error {
	...
	if fileExists(p.path) {
		...
		Trace(5, "Local checksum: ", p.checksum, " server: ", p.serverChecksum)
		if p.checksum == p.serverChecksum {
			Trace(4, "Package ", p.name, " checksums match.")
			return nil
		}
	} else {
	...
	p.updated = true
	Trace(4, "Downloaded package ", p.name, " successfully.")
	return nil
}
...
func (p *packageInfo) checkLastRunTime() {
	...
	Trace(5, "lastRunPath: ", lastRunPath)
	if !fileExists(lastRunPath) {
		p.lastRunTime = time.Time{}
		Trace(4, "lastRunPath didnt exist")
		return
	}
	...
	Trace(5, "cleanContent: ", cleanContent)
...
func (p *packageInfo) downloadPackage(a *AgentData) error {
	...
	Trace(5, "Creating the destinationfile")
	...
	Trace(5, "Receiving chunks in a loop")
	var bytesReceived int64
	for {
		chunk, err := stream.Recv()
		if err == io.EOF {
			Trace(5, "Received EOF")
	...
	Trace(5, fmt.Sprintf("Successfully downloaded %s (%d bytes)", p.name, bytesReceived))
	return nil
}
...
func (p *packageInfo) executePackageScript(a *AgentData) error {
	Trace(5, "Executing install script for ", p.name)
	...
	Trace(5, "Running script ", commandToRun, " as user: ", p.runAsUser)
	output, err := cmd.CombinedOutput()
	...
	Trace(5, "Script ", commandToRun, " ran successfully!")
